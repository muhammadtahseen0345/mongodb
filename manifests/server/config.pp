...
class mongodb::server::config {
  ...
  $journal_compressor_value = $mongodb::server::journal_compressor_value
  ...
  if ($ensure == 'present' or $ensure == true) {
    ...
    if $config_content {
    ...
    } elsif $version and (versioncmp($version, '4.0.0') >= 0) {
      # Template uses:
      # - $auth
      # - $bind_ip
      # - $configsvr
      ...
      # - $verbose
      # - $verbositylevel
      $cfg_content = template('mongodb/mongodb.4.2.yaml.erb')
    } else {
      # Fall back to mongodb.conf.2.6.erb if version is less than 4.0
      # Template uses:
      # - $auth
      # - $bind_ip
      # - $configsvr
